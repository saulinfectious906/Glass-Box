...
class EpistemicGovernorV20:
    def __init__(self):
        print("--- [SYSTEM]: Loading Dual-Brain Holographic Engine (Index 20)... ---")
        ...
        self.log_filename = f"telemetry_v20_{timestamp}.csv"
    ...
    def execute_mechanic(self, user_prompt, target_file):
        """The Back-End: Sandbox Fixed, Anti-Junk-Drawer Enforced, Dual Armor Active."""
        ...
        mechanic_prompt = (
            ...
            "CRITICAL: Do NOT reassign the 'state' variable to a new object. Only modify its keys.\n"
            "CRITICAL: The keys 'topic', 'user_facts', and 'holographic_headers' MUST remain in the dictionary.\n"
        ...
        try:
            ...
            local_scope = {"read_artifact": read_artifact, "write_artifact": write_artifact, "state": self.dynamic_state.copy()}
            
            exec(code, safe_globals, local_scope)
            
            new_state_candidate = local_scope.get('state')
            
            # --- 1. THE SCHEMA ARMOR PATCH ---
            if not isinstance(new_state_candidate, dict):
                return False, "Mechanic Error: Schema Corruption (State was overwritten by a non-dictionary object)."
                
            required_keys = ["topic", "user_facts", "holographic_headers"]
            for k in required_keys:
                if k not in new_state_candidate:
                    return False, f"Mechanic Error: Schema Corruption (Core key '{k}' was deleted)."

            # --- 2. THE JSON ARMOR PATCH ---
            try:
...
def run_v20():
    ...
    print("\n--- INDEX 20: DUAL-BRAIN GOVERNOR ONLINE ---")
    step = 0
