...
class EpistemicGovernor:
    def __init__(self, original_prompt, run_type):
        ...
        # Load Local Embedding Model (all-MiniLM-L6-v2 is fast and accurate)
        print(f"--- [SYSTEM]: Loading Local Embedding Engine... ---")
        self.embedder = SentenceTransformer('all-MiniLM-L6-v2')
        
        # Establish Semantic North locally
    ...
    def _get_embedding(self, text):
        # This now happens on your machine, not Google's servers.
    ...
    def _log_vitals(self, sim, decay, syntax, refresh, silent):
        ...
        if not silent:
            ...
            print(f"  > Similarity: {sim:.4f} (Local Vector)")
...
def run_stress_test(iterations=100, managed=False):
    ...
    print(f"\n--- STARTING {run_label} RUN (LOCAL EMBEDDING) ---")
